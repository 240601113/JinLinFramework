...
public class ExcelTools : EditorWindow
{
	...
	private static void Convert()
	{
		foreach(string assetsPath in excelList)
		{
            ...
            string savePath = Application.streamingAssetsPath + "/Json/";
            ...
            if (indexOfFormat == 0)
            {
                output = System.IO.Path.Combine(savePath, fileName + ".json");
                excel.ConvertToJson(output, encoding);
            }
            else if (indexOfFormat == 1)
            {
                output = System.IO.Path.Combine(savePath, fileName + ".csv");
                excel.ConvertToCSV(output, encoding);
            }
            else if (indexOfFormat == 2)
            {
                output = System.IO.Path.Combine(savePath, fileName + ".xml");
